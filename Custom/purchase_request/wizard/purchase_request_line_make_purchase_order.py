# Copyright 2020-2021 Barokah Perkasa Group
...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def _prepare_purchase_order_line(self, po, item):
        ...
        vals = {
            "name_product": item.line_id.name,
            "order_id": po.id,
            "product_id": product.id,
            "product_uom_id": item.line_id.product_uom_id.id,
            "price_unit": 0.0,
            # ini perubahan sementara
            #tambahan baru
            "origin": item.line_id.origin,
            "date_start": item.line_id.date_start,
            "request_accepted_date": item.line_id.request_accepted_date,
            "arrival_estimation": item.line_id.arrival_estimation,
            #tambahan baru sampai sini
            "project_id": item.line_id.project_id.id,
            "requested_by": item.line_id.requested_by.id,
            "specs": item.line_id.specs,
            "vessel_id": item.line_id.vessel_id.id,
            "form_type": item.line_id.form_type,
            "merk_barang": item.line_id.merk_barang,
            "picking_type_id": item.line_id.picking_type_id.id,
            "model_barang": item.line_id.model_barang,
            "nomor_seri": item.line_id.nomor_seri,
            "divisi_id": item.line_id.divisi_id.id,
            "code_budget_id": item.line_id.code_budget_id.id,
            "lokasi_id": item.line_id.lokasi_id.id,
            "priority": item.line_id.priority,
            "location_dest_id": item.line_id.location_dest_id.id,
            # sampai sini
            "product_qty": qty,
...
# ini yang mau diedit
    def make_purchase_order(self):
        ...
        vendor = False
        purchase = False
        resu = self.env['res.users'].browse(self.env.uid)
        employ = self.env['hr.employee'].search([('user_id', '=', resu.id)])
        if employ.department_id.name != "CORPORATE LOGISTIK":
            raise UserError(
                    _("You cannot make this order. Contact Corporate Logistic")
                )
        for item in self.item_ids:
            ...
            if self.purchase_order_id:
                ...
                line.purchase_order_id = purchase
                vendor = self.supplier_id
                line.partner_id = vendor
            if not purchase:
